...
**Current Version**: v0.8.0 · **Status**: Observability / Run History UI · **Next**: unscheduled (see [Roadmap](#roadmap))
...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Version**                      | v0.8.0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Stage**                        | Visual editor, multi-tenant auth, multi-provider AI, recurring Schedules, workflow listing/reopening, and an in-app run history UI all shipped                                                                                                                                                                                                                                                                                                                                                                                                       |
| **What works today**             | `Webhook → (Claude \| OpenAI \| Gemini) → Slack` authored visually, workspace-scoped auth, a "My Workflows" list (with a last-run success/failure indicator) to reopen and edit any previously-saved Workflow, and — new this release — clicking Execute shows the run go from RUNNING to a terminal status live in the editor, with a full history of past runs and per-step detail (status/output/error) right on the Workflow's own page; any AI step with no matching API key runs against a clearly-labeled `MockAIProvider` instead of failing |
| **What doesn't exist yet**       | Deleting a Workflow, automatic retry/backoff on execution failure, timezone support, pausing a Schedule, runtime fallback between AI providers, additional triggers/destinations, billing                                                                                                                                                                                                                                                                                                                                                            |
